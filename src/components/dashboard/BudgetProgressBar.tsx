...
export const BudgetProgressBar: React.FC = () => {
  ...
  if (
    ...
    budgetProgress.status === "critical"
  ) {
  ...
  const cappedPercentage = Math.min(
    Math.max(budgetProgress.percentage, 0),
    100,
  );
    ...
    <div className="rounded-2xl glass-card p-6 border border-slate-800 shadow-card space-y-4">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
            ...
            <div className="flex items-center gap-2 mt-0.5">
              <span className="text-xs font-mono text-slate-400">
                <strong className="text-slate-200">
                  {formatRupiah(budgetProgress.spent)}
                </strong>{" "}
                / {formatRupiah(budgetProgress.totalBudget)}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          {/* Status Badge */}
          <div
            className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold border ${config.badgeBg}`}
          >
            <StatusIcon className="w-3.5 h-3.5" />
            <span>
              {config.label} ({budgetProgress.percentage}%)
            </span>
          </div>
      ...
      {/* Feedback Message */}
      <div
        className={`p-3 rounded-xl text-xs flex items-center gap-2 ${config.badgeBg}`}
      >
