...
export function exportToExcel({
  ...
  const overviewData = [
    ...
    { Parameter: "", Nilai: "" },
    {
      Parameter: "Total Pemasukan (Income)",
      Nilai: formatRupiah(summary.income),
    },
    {
      Parameter: "Total Pengeluaran (Expense)",
      Nilai: formatRupiah(summary.expense),
    },
    {
      Parameter: "Saldo Bersih (Net Balance)",
      Nilai: formatRupiah(summary.balance),
    },
    {
      Parameter: "Alokasi Budget Bulanan",
      Nilai: formatRupiah(summary.budget),
    },
    { Parameter: "Sisa Budget", Nilai: formatRupiah(summary.remainingBudget) },
    {
      Parameter: "Status Budget",
      Nilai: summary.budgetProgress?.status?.toUpperCase() || "SAFE",
    },
    {
      Parameter: "Persentase Terpakai",
      Nilai: `${summary.budgetProgress?.percentage || 0}%`,
    },
...
export function exportToPDF({
  ...
  doc.setFontSize(9);
  doc.setFont("helvetica", "bold");
  doc.text(`PERIODE: ${monthName.toUpperCase()}`, pageWidth - 14, 16, {
    align: "right",
  });
  ...
  doc.text(
    `User: ${userName} | Dibuat: ${new Date().toLocaleDateString("id-ID")}`,
    pageWidth - 14,
    23,
    { align: "right" },
  );
  ...
  const metrics = [
    {
      label: "Pemasukan",
      val: formatRupiah(summary.income),
      color: [16, 185, 129],
    },
    {
      label: "Pengeluaran",
      val: formatRupiah(summary.expense),
      color: [244, 63, 94],
    },
    {
      label: "Sisa Saldo",
      val: formatRupiah(summary.balance),
      color: [59, 130, 246],
    },
    {
      label: "Budget Limit",
      val: formatRupiah(summary.budget),
      color: [168, 85, 247],
    },
  ];
