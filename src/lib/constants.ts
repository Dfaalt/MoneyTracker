...
export const EXPENSE_CATEGORIES: Category[] = [
  ...
  },
  {
    id: "personal",
    name: "Personal",
    icon: "/icons/person-walking.json",
    type: "expense",
    color: "#EC4899",
...
const EXPENSE_POOL = [
  ...
  { category: "Transportation", desc: "GrabCar ke Meeting", amount: 65000 },
  {
    ...
    desc: "Isi Bensin Pertamax & Tol",
    amount: 230000,
  },
  {
    category: "Transportation",
    desc: "Service & Ganti Oli Motor",
    amount: 175000,
  },
  {
    category: "Transportation",
    desc: "Tiket KRL & MRT Commuter",
    amount: 25000,
  },
  ...
  {
    category: "Quota",
    desc: "Paket Internet Bulanan Telkomsel",
    amount: 115000,
  },
...
const BONUS_INCOME_POOL = [
  {
    category: "Freelance",
    desc: "Website Landing Page Project",
    amount: 1500000,
  },
    ...
    for (let m = 1; m <= 12; m++) {
      ...
      for (let i = 0; i < count; i++) {
        const itemIdx = Math.floor(
          pseudoRandom(monthSeed * 23 + i * 7) * EXPENSE_POOL.length,
        );
      ...
      if (pseudoRandom(monthSeed * 47) > 0.35) {
        const bonusIdx = Math.floor(
          pseudoRandom(monthSeed * 59) * BONUS_INCOME_POOL.length,
        );
...
export const generateDemoBudgets = (): Budget[] => {
  ...
  for (let year = currentYear - 1; year <= currentYear + 1; year++) {
    for (let m = 1; m <= 12; m++) {
      const monthStr = `${year}-${String(m).padStart(2, "0")}`;
      const monthSeed = year * 100 + m;
      const budgetIdx = Math.floor(
        pseudoRandom(monthSeed * 83) * BUDGET_OPTIONS.length,
      );
