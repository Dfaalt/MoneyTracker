...
export const ReportsPage: React.FC = () => {
  const { summary, categorySummaries, selectedMonth, monthlyTransactions } =
    useFinance();
  ...
  const savingsRate =
    summary.income > 0
      ? Math.round(
          ((summary.income - summary.expense) / summary.income) * 1000,
        ) / 10
      : 0;
    ...
    <div className="space-y-8 animate-fade-in pb-16">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-0.5">
            Analisis mendalam pola pengeluaran dan pemasukan periode{" "}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="p-5 rounded-2xl glass-card border border-slate-800 space-y-2">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <h4 className="text-xl font-bold text-white truncate">
              {largestCategory?.category || "None"}
            </h4>
          </div>
          <p className="text-xs font-mono text-emerald-400">
            {largestCategory
              ? `${formatRupiah(largestCategory.total)} (${largestCategory.percentage}%)`
              : "Rp0"}
          </p>
        </div>
        ...
        <div className="p-5 rounded-2xl glass-card border border-slate-800 space-y-2">
          ...
          <h4
            className={`text-xl font-bold font-mono ${savingsRate >= 0 ? "text-emerald-400" : "text-rose-400"}`}
          >
            {savingsRate}%
          </h4>
          <p className="text-xs text-slate-400">
            {savingsRate >= 20
              ? "Pola tabungan sangat baik"
              : "⚠️ Tingkatkan alokasi tabungan"}
          </p>
        </div>
            ...
            <tbody className="divide-y divide-slate-800/60 font-mono">
              {categorySummaries.map((cat) => (
                ...
                >
                  ...
                  <td className="py-3 px-4 text-center text-slate-400 font-sans">
                    {cat.count}
                  </td>
